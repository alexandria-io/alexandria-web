...
jQuery(document).ready(function($){
  ...
  // make sure mobile dropdown menu disapears when an item is selected
...
function loadArtifactView(objMeta) {
  ...
  // customize the ui for music - xocel
  if (mediaType == 'music') {
    if(mediaInfo['extra-info']['coverArt']) {
      $('#album-art').attr('src', 'http://ipfs.alexandria.media/ipfs/' + mediaTid + '/' + mediaInfo['extra-info']['coverArt']);
    } else {
      $('.album-art').hide();
    }
    $('#media-column').addClass("col-sm-4 col-sm-offset-4 col-xs-12");
    $('.entity-pub-time').hide();
    $('.entity-runtime').hide();
  } else {
    $('.album-art').hide();
  }
  ...
  if (mediaArtist) {
    document.getElementById('media-artist-name').outerHTML = '<strong class="artist-title" id="media-artist-name" onclick="searchByField(&apos;media&apos;, &apos;info_extra&apos;, &apos;'+ mediaArtist +'&apos;)">'+ mediaArtist +'</strong>';
  }
